...
5. Use **Reset demo** again after each test. PROMPT pins the selected source commit, renames the old demo, creates a fresh one, and requests a move of the old project into the private `demo-archives` subgroup. If GitLab denies that move, the reset still succeeds and PROMPT shows the old project's link; a GitLab Owner must move it from the active Introcourse group into `demo-archives`. Repeat this cycle until the clean demo, CI, access, and issues all pass the live readiness check.
...
- Archive projects are retained for audit and repeated experiments. Review and remove old archives only after their results are no longer needed. Reset itself does not delete them. A GitLab Owner must complete any archive moves that the PROMPT service account cannot perform.
